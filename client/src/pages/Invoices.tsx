...
import { Search, Plus, FileText, Printer, X, RefreshCw, Copy, Check } from 'lucide-react';
...
export const Invoices: React.FC = () => {
  ...
  const handleCreateInvoice = async (e: React.FormEvent) => {
    ...
    if (!selectedStudentId || selectedFeeTypeIds.length === 0 || !dueDate) {
      push('warning', 'Please select student, fee heads, and due date');
      return;
    }
  ...
  const copyInvoiceNo = (no: string) => {
    navigator.clipboard.writeText(no);
    setCopiedNo(no);
    push('info', `Copied ${no} to clipboard`);
      ...
      <div className="bg-white rounded-3xl p-6 border border-[#E5E7EB] card-shadow">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {['ALL', 'UNPAID', 'PARTIAL', 'PAID', 'OVERDUE'].map((st) => (
              <button
                ...
                className={`px-3 py-1.5 min-h-[36px] rounded-full text-xs font-extrabold transition-colors ${
                  selectedStatus === st
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-xs text-[#6B7280] mt-1">Try clearing filters or create a new invoice.</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-[#E5E5E5] text-xs">
                {filteredInvoices.map((inv) => (
                  <tr key={inv.id} className="hover:bg-[#F5F5F0] transition-colors">
                    <td className="py-3.5 pl-2 font-mono font-bold text-[#1A1A1A]">
                      <button
                        onClick={() => copyInvoiceNo(inv.invoiceNo)}
                        className="hover:text-[#E85D04] inline-flex items-center gap-1.5 transition-colors"
                        title="Click to copy invoice number"
                      >
                        <span>{inv.invoiceNo}</span>
                        {copiedNo === inv.invoiceNo ? (
                    ...
                    <td className="py-3.5 text-right pr-2">
                      <button
                        ...
                        aria-label={`View PDF for ${inv.invoiceNo}`}
                        className="p-2 min-w-[36px] min-h-[36px] text-[#6B7280] hover:text-[#E85D04] hover:bg-[#FFF0E6] rounded-xl transition-colors inline-flex items-center justify-center"
          ...
          <div className="bg-white rounded-3xl p-6 max-w-md w-full border border-[#E5E7EB] shadow-2xl">
            ...
            <form onSubmit={handleCreateInvoice} className="space-y-4 pt-4 text-xs">
              ...
              <div>
                ...
                <div className="max-h-36 overflow-y-auto custom-scrollbar bg-[#F5F5F0] border border-[#E5E7EB] rounded-xl p-2 space-y-1">
                  {feeTypes.map((ft) => (
                    <label key={ft.id} className="flex items-center gap-2 p-2 hover:bg-white rounded-lg cursor-pointer min-h-[36px]">
                      <input
                        type="checkbox"
                        checked={selectedFeeTypeIds.includes(ft.id)}
                        onChange={() => handleFeeTypeToggle(ft.id)}
                        className="rounded border-[#E5E7EB] text-[#E85D04] focus:ring-[#E85D04] w-4 h-4"
                      />
                      <span className="font-semibold text-[#1A1A1A] flex-1">{ft.name}</span>
                      <span className="font-mono font-bold text-[#E85D04]">{formatCurrency(ft.amount)}</span>
                    </label>
                  ))}
                </div>
              </div>
          ...
          <div className="bg-white rounded-3xl p-8 max-w-lg w-full border border-[#E5E7EB] shadow-2xl">
            <div className="flex items-center justify-between pb-4 border-b border-[#E5E7EB] print:hidden">
              <span className="text-xs font-extrabold text-[#16A34A] bg-emerald-50 px-3 py-1 rounded-full border border-emerald-200">
                Official PDF Tax Invoice
              </span>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => window.print()}
                  className="flex items-center gap-1.5 px-4 py-2 min-h-[36px] bg-[#F5F5F0] border border-[#E5E7EB] rounded-xl text-xs font-bold text-[#1A1A1A] hover:border-[#E85D04] active:scale-[0.98] transition-all"
                >
            ...
            <div className="pt-6 space-y-6 text-xs text-[#1A1A1A]">
              ...
              <div className="bg-[#F5F5F0] p-4 rounded-2xl border border-[#E5E7EB] grid grid-cols-2 gap-4">
                <div>
                  ...
                  <span className="text-[11px] text-[#6B7280] font-mono">Code: {activeInvoiceModal.student?.studentCode}</span>
                </div>
                ...
                <div>
                  ...
                  <a href={`tel:${activeInvoiceModal.student?.parentPhone}`} className="text-[11px] text-[#6B7280] hover:text-[#E85D04]">
                    {activeInvoiceModal.student?.parentPhone}
                  </a>
                </div>
              </div>
              ...
              <div className="border-t border-b border-[#E5E7EB] py-4 space-y-2">
                ...
                {activeInvoiceModal.items && activeInvoiceModal.items.length > 0 ? (
                  ...
                  <div className="flex justify-between font-semibold">
                    ...
                    <span className="font-mono">{formatCurrency(activeInvoiceModal.totalAmount)}</span>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center justify-between pt-2">
                ...
                <div className="text-right">
                  ...
                  <span className="text-2xl font-mono font-extrabold text-[#E85D04] tracking-tight">
